...
# Fisher' z test: the significance of each pair of correlations

compare_correlations <- function(r1, r2, n1, n2, alpha = 0.05) {
  ...
  result_str <- sprintf("Z = %.2f (p = %.3f)%s", 
                       z_stat, 
                       p_value,
...
compare_correlation_matrices <- function(cor1, cor2, n1, n2) {
  if(!all(dim(cor1) == dim(cor2))) {
    stop("Dimension not match")
  }
  
  nvar <- ncol(cor1)
  result_matrix <- matrix("", nvar, nvar) 
  
  rownames(result_matrix) <- colnames(cor1)
  colnames(result_matrix) <- colnames(cor1)
  
  for(i in 1:(nvar-1)) {
    for(j in (i+1):nvar) {
      result <- compare_correlations(cor1[i,j], cor2[i,j], n1, n2)
      result_matrix[i,j] <- result_matrix[j,i] <- result$formatted
    }
  }
...
n1 <- 93
n2 <- 184
results <- compare_correlation_matrices(cor_psy, cor_credamo, n1, n2)
print(results)
